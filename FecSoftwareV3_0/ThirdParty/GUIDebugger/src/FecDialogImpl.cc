...
FecDialogImpl::FecDialogImpl ( FecAccess *fecAccess, std::string partitionName, std::string fecHardwareId ):
  ...
  if (login == "nil") {
    ...
    try {
      std::cout << "Create an access to the database "
		<< login << "@" << path << std::endl ;
    ...
    if (index != 0) {
      ...
      if (sscanf (i2cAddressAccess->text(), "%x", &address)) {
        ...
        if (i2cAndOperation->isChecked() ||
          ...
          sprintf (msg, "0x%04X", value) ;
          i2cDataAccess->setText (msg) ;
	  std::cout << "Read mask register Not implemented" << std::endl ;
        }
      }
      else {
...
void FecDialogImpl::i2cReadModifyWrite() {
  ...
  try {
	...
	std::cout << "Read modify write is Not implemented" << std::endl ;
...
void FecDialogImpl::emptyFIFOs ( FecAccess *fecAccess_, keyType index, bool display ) {
  ...
  try {
    ...
    if (! isFecSR0Correct(fecAccess_->getFecRingSR0(index))) {
	  ...
	  if (display) 
	    std::cout << "FIFO receive word(" << dec << loopCnt << ") = " 
...
// 	} // end of while loop 
	...
	while (
	  ...
	  if (display) 
	    std::cout << "FIFO transmit word(" << dec << loopCnt << ") = " 
  ...
  if (display)
    std::cout << "Stop emptyFIFOs: SR0 = 0x" << hex << fecAccess_->getFecRingSR0(index) << std::endl ;
}
