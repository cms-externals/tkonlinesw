...
FecRingDevice *FecAccess::setFecRingDevice ( keyType index ) {
  ...
  if (fecPair == fecRingEnable_.end()) {
	...
	try {
	  ...
	  fec = new FecVmeRingDevice ( hardwareDevices_[getFecKey(index)], getFecKey(index), getRingKey(index), initFecRingDevice_, blockModeE, adapterSlot_, invertClockPolarity_ ) ;
...
void FecAccess::crateReset ( ) throw (FecExceptionHandler) {
  ...
  if (p!=fecRingEnable_.end()) {
    ...
    ((FecVmeRingDevice *)(p->second))->crateReset(adapterSlot_) ;
...
void FecAccess::seti2cChannelCRA( keyType index, tscType8 value,logicalOperationType op) throw ( FecExceptionHandler ) {
  ...
  // Hardware access
  fec->seti2cChannelCRA ( index, value,op) ;
}
...
FecAccess *FecAccess::createFecAccess ( int argc, char **argv, int *cnt, bool init ) throw (FecExceptionHandler) {
  ...
  for (int i = 0 ; i < argc ; i ++) {
    ...
    else if (strcasecmp(argv[i],"-adapterSlot") == 0) {
      if ((i+1) < argc) {
	i ++ ;
	if (sscanf (argv[i], "%u", &adapterSlot) == 0) {
	  std::cerr << "Please specify the adapter slot between 0 and 3" << std::endl ;
	  return NULL ;
	}
      }
      else {
	std::cerr << "Please specify the adater slot after option -adapterSlot" << std::endl ;
	return NULL ;
      }
    } 
  ...
  case FECVME: {
    ...
    if (pnpUsed) {
      fecAccess = new FecAccess (adapterSlot, vmeFileName, vmeFileNamePnP, forceAck, initFec, true, false, (tscType16)i2cSpeed, FecVmeRingDevice::STR_BUSADAPTER_NAME[busAdapter], false, 8, invertClockPolarity) ;
    }
    else {

      fecAccess = new FecAccess (adapterSlot, vmeFileName, fecAddresses, forceAck, initFec, true, false, (tscType16)i2cSpeed, FecVmeRingDevice::STR_BUSADAPTER_NAME[busAdapter], false, 8, invertClockPolarity) ;
    }
    break ;
  }
  case FECUSB:
