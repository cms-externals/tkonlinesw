...
/*     * Module on Fec 0x6 Ring 0x0 Ccu 0x60 I2c channel 0x11 I2c address 0x0 */
	...
	if (apv6F || apv4F || pllF || muxF || aohF) {
	  bool error = false ;
	  if (displayMessage)
	    std::cout << "Found a module on FEC " << std::dec << (int) getFecKey(indexF) << " Ring " << (int) getRingKey(indexF) << " CCU 0x" << std::hex << (int) getCcuKey(indexF) << " channel 0x" << std::hex << (int) getChannelKey(indexF) << std::dec << std::endl ;
	  ...
	  if ( dcu0F && !apv6F && !apv4F && !pllF && !muxF && !aohF ) {
	    ...
	    if (dcuOnCcuTag) {
	      ...
	      if (displayMessage) {
		std::cout << "Found a DCU on CCU on FEC " << (int) getFecKey(indexF) << " Ring " << (int) getRingKey(indexF) << " CCU 0x" << std::hex << (int) getCcuKey(indexF) << " channel 0x" << std::hex << (int) getChannelKey(indexF) << std::dec << std::endl;
  ...
  static void getCrateFecSupervisorInstance ( FecAccess &fecAccess, keyType fecInstance[MAX_NUMBER_OF_SLOTS], 
	...
	for (int i = 0 ; i < MAX_NUMBER_OF_SLOTS ; i ++) {
	  if (fecInstance[i] != NOFECRING) {
	    ...
	    if ((partitionName != "") && (partitionName != "nil"))
	      *instanceStream << fecHardwareId << "\t" << (int) getFecKey(fecInstance[i]) << "\t" << i << "\t" << partitionName << std::endl ;
	    else
	      *instanceStream << fecHardwareId << "\t" << (int) getFecKey(fecInstance[i]) << "\t" << i << std::endl ;
	  }
	}
      }
    }
...
/*       std::cout << "ptr = " << memDevices << std::endl ; */
	...
	if (fecAccess.getFecBusType() == FECVME) {
	  if (fecHardwareIdArray[getFecKey(index)] == "") {
	    try {
	      ...
	      msgError << "Cannot read the FEC Hardware id from FEC " << std::dec << (int) getFecKey(index) << ": " << e.what() ;
	      listError.push_back(msgError.str()) ;
	    }
	  }
  ...
  static bool uploadDetectFECs ( 
	...
	try {
	  ...
	  std::ostringstream msgInfo ; msgInfo << "Reset the FEC on ring " << std::dec << (int) getFecKey((*it)) << "." << (int) getRingKey((*it)) ;
	  errorReportLogger.errorReport (msgInfo.str(), LOGDEBUG) ;
	}
	catch (FecExceptionHandler &e) {
	  std::ostringstream msgError ; msgError << "FEC: Unable to reset the FEC on ring " << std::dec << (int) getFecKey((*it)) << "." << (int) getRingKey((*it)) ;
	...
	if (fecAccess.getFecBusType() == FECVME) {
	  try {
	    ...
	    std::ostringstream msgError ;
	    msgError << "Cannot read the FEC Hardware id from FEC " << std::dec << (int) getFecKey(it->first) << ": " << e.what() ;
	    listErrors.push_back(msgError.str()) ;
	  }
	}
  ...
  static bool setRedundancy ( FecAccess &fecAccess, keyType index, TkRingDescription &tkRing, bool forceApplyRedundancy, ErrorReportLogger &errorReportLogger ) {
	...
	if (isLinkInit(fecAccess.getFecRingSR0(index)) && 
	  ...
	  if (isFecSR0Correct(fecSR0)) {
	    std::stringstream msgInfo ; msgInfo << "Ring on FEC " << (int) getFecKey(index) << "." << (int) getRingKey(index) << " SR0 = 0x" << std::hex << fecSR0 << std::dec ;
	...
	if (forceApplyRedundancy || !isFecSR0Correct(fecSR0)) {
	  ...
	  if (!tkRing.isReconfigurable()) {
	    ...
	    msgInfo << "The configuration given for FEC " << (int) getFecKey(index) << " ring " << (int) getRingKey(index) << " is not correct, please check it: " ;
	    ...
	    try {
	      std::stringstream msgInfo ;
	      msgInfo << "Apply redundancy on FEC " << (int) getFecKey(index) << "." << (int) getRingKey(index) << ": " ; tkRing.display(msgInfo) ;
	      errorReportLogger.errorReport (msgInfo.str(), LOGUSERINFO) ;
	      fecSR0 = fecAccess.fecRingReconfigure (index, tkRing) ;
	      if (!isFecSR0Correct(fecSR0)) {
		std::stringstream msgError ; 
		msgError << "The configuration given for FEC " << (int) getFecKey(index) << " ring " << (int) getRingKey(index) 
		...
		std::stringstream msgInfo ; 
		msgInfo << "The configuration given for FEC " << (int) getFecKey(index) << " ring " << (int) getRingKey(index) 
		...
		error = false ;
	      ...
	      std::stringstream msgError ; msgError << "Unable to apply the redundancy on FEC " << (int) getFecKey(index) << " ring " << (int) getRingKey(index) << ": " << e.what() ;
	      errorReportLogger.errorReport (msgError.str(), LOGERROR) ;
	      error = true ;
	    }
	  }
      ...
      std::stringstream msgError ; msgError << "Unable to retreive FEC status on FEC " << (int) getFecKey(index) << " ring " << (int) getRingKey(index) << ": " << e.what() ;
      errorReportLogger.errorReport (msgError.str(), LOGERROR) ;
      error = true ;
    }
